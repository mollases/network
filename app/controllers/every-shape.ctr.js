...
angular.module('network.shapeFinderCtrl', ['ngRoute', 'ngVis'])

.config(['$routeProvider',
    function($routeProvider) {
        $routeProvider.when('/every-shape', {
            templateUrl: 'views/network.tpl.html',
            controller: 'shapeFinderCtrl'
        });
    }
])

.controller('shapeFinderCtrl', ['$scope', 'NetworkGeneratorSvs', 'HelperService',
    function($scope, NetworkGeneratorSvs, HelperService) {
        $scope.challenge = 'Find Every Unique cycle';
        $scope.messages = [];
        var opts = {
